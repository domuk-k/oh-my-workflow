...
| Hook | Role |
|------|------|
| `agent(prompt, opts?)` | Run one CLI node. With `schema`, validates JSON and **re-prompts on failure** (up to 2 retries). Ordinary failures return `null`; a direct budget exhaustion throws. |
